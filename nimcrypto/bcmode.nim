...
proc encrypt*[T](ctx: var ECB[T], inp: ptr byte, oup: ptr byte,
  ...
  mixin encrypt
  doAssert(not isNil(inp) and not isNil(oup))
  doAssert(length mod uint(ctx.sizeBlock) == 0)
...
proc init*[T](ctx: var CBC[T], key: ptr byte, iv: ptr byte) =
  ...
  mixin init
  doAssert(not isNil(key) and not isNil(iv))
  init(ctx.cipher, key)
  doAssert(ctx.sizeBlock <= MaxBlockSize)
...
proc encrypt*[T](ctx: var CTR[T], inp: ptr byte, oup: ptr byte,
  ...
  doAssert(not isNil(inp) and not isNil(oup))
  doAssert(length != 0)
  doAssert(ctx.sizeBlock == (128 div 8) or ctx.sizeBlock == (256 div 8))
  var n = ctx.num
  var i = 0'u
  var ip = cast[ptr UncheckedArray[byte]](inp)
  var op = cast[ptr UncheckedArray[byte]](oup)
