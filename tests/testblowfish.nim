...
when isMainModule:
  ...
  for i in 0..<NUM_VARIABLE_KEY_TESTS:
    ...
    # initBlowfishContext(ctx, len(key) * 8, addr key[0])
    var data = [plaintext_l[i], plaintext_r[i]]
    ctx.encrypt(cast[ptr uint8](addr data[0]), cast[ptr uint8](addr data[0]))
    # blowfishEncrypt(ctx, cast[ptr uint8](addr data[0]))
    if ciphertext_l[i] != data[0] and ciphertext_r[i] != data[1]:
      echo "BROKEN test [1] at iteration [" & $i & "]"
  ...
  for i in 0..<NUM_VARIABLE_KEY_TESTS:
    ...
    # blowfishDecrypt(ctx, cast[ptr uint8](addr data[0]))
    if plaintext_l[i] != data[0] and plaintext_r[i] != data[1]:
      echo "BROKEN test [2] at iteration [" & $i & "]"
  ...
  for i in 0..<NUM_SET_KEY_TESTS:
    ...
    ctx.encrypt(cast[ptr uint8](addr data[0]), cast[ptr uint8](addr data[0]))
    # blowfishEncrypt(ctx, cast[ptr uint8](addr data[0]))
    if ciphertext_l[NUM_VARIABLE_KEY_TESTS + i] != data[0] and
       ciphertext_r[NUM_VARIABLE_KEY_TESTS + i] != data[1]:
      echo "BROKEN test [3] at iteration [" & $i & "]"
  ...
  for i in 0..<NUM_SET_KEY_TESTS:
    ...
    ctx.decrypt(cast[ptr uint8](addr data[0]), cast[ptr uint8](addr data[0]))
    # blowfishDecrypt(ctx, cast[ptr uint8](addr data[0]))
    if plaintext_l[NUM_VARIABLE_KEY_TESTS + i] != data[0] and
       plaintext_r[NUM_VARIABLE_KEY_TESTS + i] != data[1]:
      echo "BROKEN test [4] at iteration [" & $i & "]"
